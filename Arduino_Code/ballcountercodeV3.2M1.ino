...
// SPEED / TIMING CONFIG
// ============================================================
const float BEAM_SPACING_IN = 1.61;
...
// 1. Direct raw-speed calibration
// 2. Smoothed final correction based on the latest non-raw validation data
//
// For your second counter later, this is the main section to swap.
// ============================================================

// ============================================================
// DIRECT RAW-SPEED CALIBRATION FUNCTION
// ============================================================
// Raw speed is calculated from dtUs using BEAM_SPACING_IN = 1.61.
//
// New permanent-mount direct calibration points:
//
// raw 31.504 mph -> calibrated 35.105 mph
// raw 50.404 mph -> calibrated 52.305 mph
// raw 61.649 mph -> calibrated 64.020 mph
float calibrateSpeed(float rawMph) {
  const int N = 3;

  const float rawPts[N] = {
    31.504,
    50.404,
    61.649
  };

  const float calPts[N] = {
    35.105,
    52.305,
    64.020
  };
  ...
  if (rawMph <= rawPts[0]) {
    float slope = (calPts[1] - calPts[0]) / (rawPts[1] - rawPts[0]);
    return calPts[0] + slope * (rawMph - rawPts[0]);
  }

  if (rawMph >= rawPts[N - 1]) {
    float slope = (calPts[N - 1] - calPts[N - 2]) / (rawPts[N - 1] - rawPts[N - 2]);
    return calPts[N - 1] + slope * (rawMph - rawPts[N - 1]);
  }

  for (int i = 0; i < N - 1; i++) {
    if (rawMph >= rawPts[i] && rawMph <= rawPts[i + 1]) {
      float fraction = (rawMph - rawPts[i]) / (rawPts[i + 1] - rawPts[i]);
      return calPts[i] + fraction * (calPts[i + 1] - calPts[i]);
    }
  }
...
// ============================================================
// SMOOTHED FINAL CORRECTION FUNCTION
// ============================================================
// Input: current calibrated speed from calibrateSpeed()
// Output: final displayed speed
//
// This correction is based on the latest non-raw validation data.
// It uses 4 regions but smooths the transition areas to avoid harsh jumps.
// ============================================================
// SMOOTHED FINAL CORRECTION FUNCTION
// ============================================================
// Input: current calibrated speed from calibrateSpeed()
// Output: final displayed speed
//
// This correction uses two linear fits with a smooth transition.
// The center breakpoint is 56.66 mph.
// The transition blends across +/- 1.0 mph around the breakpoint.
float finalCorrection(float currentCalMph) {
  const float BREAKPOINT = 56.66;
  const float BLEND_WIDTH = 1.0;   // transition half-width in mph

  // Low/mid-speed line
  float lowLine = 1.05848 * currentCalMph - 2.41925;

  // High-speed line
  float highLine = 0.43611 * currentCalMph + 35.43995;

  // Fully low side
  if (currentCalMph <= BREAKPOINT - BLEND_WIDTH) {
    return lowLine;
  }
  ...
  // Smooth transition between the two lines
  float t = (currentCalMph - (BREAKPOINT - BLEND_WIDTH)) / (2.0 * BLEND_WIDTH);
  t = t * t * (3.0 - 2.0 * t);   // smoothstep

  return lowLine + (highLine - lowLine) * t;
}
